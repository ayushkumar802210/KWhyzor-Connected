...
async function loadUserData(){
  ...
  if(p){state.displayName=p.display_name||"";state.phone=p.phone||"";state.location=p.location||"";state.avatarUrl=p.avatar_url||"";state.plan=p.plan||"Free";state.role=p.role||"user";state.memberSince=p.created_at||state.memberSince;if(state.avatarUrl){const signed=await sb.storage.from("avatars").createSignedUrl(state.avatarUrl,3600);if(!signed.error)state.avatarUrl=signed.data.signedUrl}}
  ...
  localStorage.setItem("kwhyzor_display_name",state.displayName);localStorage.setItem("kwhyzor_phone",state.phone);localStorage.setItem("kwhyzor_location",state.location);localStorage.setItem("kwhyzor_avatar",state.avatarUrl);localStorage.setItem("kwhyzor_member_since",state.memberSince);localStorage.setItem("kwhyzor_role",state.role);
  updateProfileMenu();
}
async function init(){
  ensureDemoUser();
...
async function saveProfile(){const button=$("#saveProfile"),name=$("#profileName").value.trim(),display=$("#profileDisplay").value.trim(),phone=$("#profilePhone").value.trim(),location=$("#profileLocation").value.trim();if(name.length<2||name.length>80){toast("Enter a name between 2 and 80 characters.");return}if(phone&&!/^[+()\-\d\s]{7,20}$/.test(phone)){toast("Enter a valid phone number.");return}button.disabled=true;button.textContent="Saving...";state.name=name;state.displayName=display;state.phone=phone;state.location=location;let avatarPath=state.avatarUrl;if(sb&&state.userId&&state.avatarUrl.startsWith("data:")){const response=await fetch(state.avatarUrl),blob=await response.blob(),path=`${state.userId}/profile-image`;const upload=await sb.storage.from("avatars").upload(path,blob,{contentType:blob.type,upsert:true});if(upload.error){button.disabled=false;button.textContent="Save Changes";toast(upload.error.message);return}avatarPath=path;const signed=await sb.storage.from("avatars").createSignedUrl(path,3600);if(!signed.error)state.avatarUrl=signed.data.signedUrl}localStorage.setItem("kwhyzor_name",name);localStorage.setItem("kwhyzor_display_name",display);localStorage.setItem("kwhyzor_phone",phone);localStorage.setItem("kwhyzor_location",location);if(!sb)localStorage.setItem("kwhyzor_avatar",state.avatarUrl);setUserUI();if(sb&&state.userId){const {error}=await sb.from("profiles").upsert({id:state.userId,full_name:name,display_name:display,phone,location,avatar_url:avatarPath||null,updated_at:new Date().toISOString()});if(error){button.disabled=false;button.textContent="Save Changes";toast(error.message);return}}button.disabled=false;button.textContent="Save Changes";toast("Profile updated successfully.");render("profile")}
...
function saveReviewedBill(){
  const amount=Number($("#scanAmount").value),units=Number($("#scanUnits").value),date=$("#scanDate").value;
  if(!amount||!units||!date){toast("Review all bill fields before saving.");return}
  state.bills.unshift({month:new Date(date).toLocaleDateString("en-US",{month:"long",year:"numeric"}),amount,units});
  if(sb&&state.userId)sb.from("bills").insert({user_id:state.userId,billing_date:date,amount,units_kwh:units,ocr_status:"reviewed"});
  toast("Reviewed bill added to your history.");render("bills");
}
function scanner(){return `<div><h1 class="page-title">Bill Scanner</h1><p class="page-copy">Upload a bill, review extracted fields, then save it. OCR is a demo adapter until a secure provider is connected.</p><section class="panel scanner-panel"><label class="upload-zone" for="billFile"><span class="upload-icon">▣</span><b>Choose an electricity bill</b><small>PDF, JPG or PNG · your file stays in this browser demo</small><input id="billFile" type="file" accept=".pdf,.jpg,.jpeg,.png" onchange="previewBill(this)"></label><div id="scanReview" class="hidden"><div class="review-heading"><div><h3>Review extracted information</h3><p class="panel-sub">Demo extraction has low confidence. Verify every value against your bill.</p></div><span class="confidence low">Low OCR confidence</span></div><div class="form-grid">${field("Billing date","scanDate","2026-08-01","date")}${field("Total amount (₹)","scanAmount","3140","number",0,100000)}${field("Units consumed (kWh)","scanUnits","412","number",0,100000)}${field("Bill / account reference","scanRef","DEMO-2026-08")}</div><div class="button-row"><button class="primary" onclick="saveReviewedBill()">Review and Save Bill</button><button class="choice" onclick="$(\"#scanReview\").classList.add(\"hidden\")">Discard</button></div></div></section><section class="panel scanner-note"><h3>Privacy and accuracy</h3><p class="panel-sub">Never rely on OCR alone for billing decisions. KWhyzor labels these values as estimates or extracted data until you verify them.</p></section></div>`}
function previewBill(input){if(input.files.length){$("#scanReview").classList.remove("hidden");toast("Bill uploaded. Review the extracted fields.")}}
async function addAppliance(){
  ...
  if(!sb||!state.userId){state.appliances.push({name,wattage,hours_per_day:hours,quantity:1,days_per_month:30});render(state.page);toast("Appliance added to your demo twin.");return}
  const {error}=await sb.from("appliances").insert({user_id:state.userId,name,wattage,hours_per_day:hours,quantity:1});
  if(error)toast(error.message);else{await loadUserData();render(state.page);toast("Appliance saved.")}
}
function dashboard(){
...
function detective(){const current=state.bills[0]||{month:"Current",amount:0,units:0},previous=state.bills[1]||{month:"Previous",amount:0,units:0};const result=KWhyzorEngine.investigate(current.units,previous.units,{coolingHours:3,pumpHours:1,otherChanges:1});return `<div><h1 class="page-title">Bill Detective 🔎</h1><p class="page-copy">Investigate changes between billing periods using your saved bill history and reported usage signals.</p><section class="panel"><div class="cards"><div class="metric"><span>${previous.month}</span><strong>₹${previous.amount.toLocaleString()}</strong><small>${previous.units} kWh</small></div><div class="metric"><span>${current.month}</span><strong>₹${current.amount.toLocaleString()}</strong><small>${current.units} kWh</small></div><div class="metric"><span>Usage change</span><strong class="up">${result.changePercent.toFixed(1)}%</strong><small>month over month</small></div><div class="metric"><span>Confidence</span><strong class="warn">${result.confidence}</strong><small>based on available data</small></div></div><div class="result"><h3>🔎 Most likely cause: ${result.leadingCause}</h3><p class="panel-sub">${result.signals[0].detail} This is an estimate, not a utility diagnosis.</p><div class="list">${result.signals.map(signal=>`<div class="list-row"><b>${signal.name}</b><span class="value">${Math.round(totalSignal(signal,result.signals))}% signal</span></div>`).join("")}</div><p class="panel-sub">Next evidence: ${result.missing}</p></div></section><section class="panel" style="margin-top:15px"><h3>Investigation questions</h3><div class="form-grid" style="margin-top:15px">${["Did you start using a new AC?","Did AC usage hours increase?","Was the water pump used more?","Did the number of occupants change?"].map(q=>`<button class="choice" style="text-align:left;padding:15px" onclick="toast('Answer recorded: ${q}')">${q}<br><span style="color:var(--cyan)">Tap to answer →</span></button>`).join("")}</div></section></div>`}
function totalSignal(signal,signals){const total=signals.reduce((sum,item)=>sum+item.score,0);return total?signal.score/total*100:0}
...
function twin(){const defaults=[{name:"Air Conditioner",category:"Cooling",wattage:1500,quantity:1,hoursPerDay:5,daysPerMonth:30},{name:"Fans",category:"Cooling",wattage:75,quantity:3,hoursPerDay:8,daysPerMonth:30},{name:"Refrigerator",category:"Kitchen",wattage:150,quantity:1,hoursPerDay:24,daysPerMonth:30}];const items=state.appliances.length?state.appliances.map(a=>({name:a.name,category:a.category||"Home",wattage:a.wattage,quantity:a.quantity,hoursPerDay:a.hours_per_day,daysPerMonth:a.days_per_month})):defaults;const total=items.reduce((sum,item)=>sum+KWhyzorEngine.applianceKwh(item),0);return `<div><h1 class="page-title">Electricity Twin 🏠</h1><p class="page-copy">Model your home with transparent appliance assumptions. Results are estimates, not meter readings.</p><section class="panel"><div class="twin" style="min-height:250px"><div class="house">🏠</div><span class="chip ac">❄️ Cooling</span><span class="chip fan">🌀 Fans</span><span class="chip ev">🚗 EV ready</span><span class="chip solar">☀️ Solar ready</span></div><div class="result"><span>Estimated monthly home demand</span><strong>${total.toFixed(1)} kWh</strong><p class="panel-sub">Calculated as watts ÷ 1,000 × quantity × hours/day × days/month.</p></div><div class="list">${items.map(item=>`<div class="list-row"><div><b>${item.name}</b><small>${item.wattage} W · ${item.quantity} unit · ${item.hoursPerDay} h/day</small></div><span class="value">${KWhyzorEngine.applianceKwh(item).toFixed(1)} kWh</span></div>`).join("")}</div><div class="button-row"><button class="primary" onclick="addAppliance()">+ Add Appliance</button><button class="choice" onclick="state.page='simulator';render('simulator')">Run What-If</button></div></section></div>`}
